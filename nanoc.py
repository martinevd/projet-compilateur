...
def asm_expression(e, name_fct):
    ...
    if e.data == "call_function_expr":
        ...
        if len(liste_exprs) != global_ctx.nb_args(fct_to_call):
            raise TypeError(
                f"La fonction '{fct_to_call}' attend {global_ctx.nb_args(fct_to_call)} arguments, mais {len(liste_exprs)} ont été fournis."
            )
...
def type_of_expression(e, name_fct):
    ...
    if e.data == "opbin":
        ...
        if type_left_exp != type_right_exp:
            if not (type_left_exp in ["int", "long"] and type_right_exp == "number"):
                raise TypeError(
    ...
    # Returns either number or double
    return e.data
...
"""
    ...
    if c.data == "affectation":
        ...
        right_exp = type_of_expression(exp, name_fct)

        if type_of_expression(exp, name_fct) != var_ctx.var_type:
            if not (var_ctx.var_type in ["int", "long"] and right_exp == "number"):
